...
namespace StoreSimpleData.Implementation
{
    internal static class ConvertItTo
    {
        ...
        {
            return new DataBS.MindReceivedData { Id = inp.Id ?? 0, Details = inp.Details, MaxTarget = inp.MaxTarget ?? 0, Score = inp.Score ?? 0, Title = inp.Title };
        }
        internal static DataBS.MindTrainedData DBMindTrainedData(MindTrainedData inp)
        {
            return new DataBS.MindTrainedData { Id = inp.Id ?? 0, Details = inp.Details, Target = inp.Target ?? 0, Approved = inp.Approved ?? false, Title = inp.Title };
        }
